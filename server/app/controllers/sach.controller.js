...
exports.getAllSach = async (req, res, next) => {
    try {
        ...
        if (name) { //findByName
            ...
            sachs = await Sach.find().populate('MANXB');
        }
...
exports.createSach = async (req, res, next) => {
    ...
    try {
        ...
        return next(
            new ApiError(400, `Có lỗi xảy ra khi tạo một đối tượng mới`)
        );
    }
};
...
exports.getSachByID = async (req, res, next) => {
    try {
        ...
        return next(
            new ApiError(500, `Có lỗi xảy ra khi tìm kiếm đối tượng có id=${req.params.id}`)
        );
    }
};
...
exports.updateSach = async (req, res, next) => {
    try {
        ...
        return next(
            new ApiError(500, `Có lỗi xảy ra khi update đối tượng có id=${req.params.id}`)
        );
    }
};
